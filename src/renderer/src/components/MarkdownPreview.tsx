...
const hljsThemes = import.meta.glob('../../../../node_modules/highlight.js/styles/github{,-dark}.css', { query: '?url' })
...
export function MarkdownPreview({ content, maxLength, variant = 'preview' }: MarkdownPreviewProps) {
  ...
  useEffect(() => { applyHljsTheme(colorScheme) }, [colorScheme])
  const clean = content.replace(/<!-- luthor:meta .*? -->/g, '').trim()
  const preview = variant !== 'full' && maxLength != null && clean.length > maxLength
  ...
  return variant === 'full'
    ? body
    : <Text size="sm" component="div">{body}</Text>
}
